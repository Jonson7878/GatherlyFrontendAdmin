    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const ordersData = Array.isArray(payload)
          ? payload
          : payload.orders || payload.Orders || payload.data?.orders || payload.data?.Orders || [];

        if (!ordersData || ordersData.length === 0) {
          setAlert({ message: "No Orders found.", severity: "info" });
          setLoading(false);
          return;
        }
    ...
    return (
      <Container maxWidth="lg" sx={{ py: 6, minHeight: "100vh" }}>
        <Typography
          variant="h4"
          align="center"
          sx={{ mb: 5, fontWeight: "bold", color: "#1976d2", fontFamily: "'Poppins', sans-serif" }}
        >
          All Orders
        </Typography>
        {alert.message && (
          ...
          <TableBody>
            {orders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((order) => (
              <TableRow key={order._id} sx={{ "&:hover": { backgroundColor: "#f5f5f5" } }}>
                ...
                <TableCell>{order.eventId?.eventName}</TableCell>
