...
const setupTwoFactorApi = async () => {
  const res = await axios.post(
    `${API_BASE}/api/user/2fa/setup`,
    {},
  ...
  return {
    qrCode: data.qrCode,
    secret: data.secret,
    otpauthUrl: data.otpauthUrl || data.otpauth_url || '',
  };
};

const verifyTwoFactorApi = async (token) => {
  const res = await axios.post(
    `${API_BASE}/api/user/2fa/verify`,
    { token },
    { headers: { 'Content-Type': 'application/json', ...getAuthHeader() } }
  );
  return res.data;
...
const validateTwoFactorApi = async ({ userId, token }) => {
  const res = await axios.post(
    `${API_BASE}/api/user/2fa/validate`,
    { userId, token },
      ...
      if (params.mode === 'setup') {
        ...
        if (res?.token && res?.user) {
          if (res.user.role !== 'admin') {
            setAlert({ message: 'Access denied: Only admin users are allowed to log in.', severity: 'error' });
            return;
          }
